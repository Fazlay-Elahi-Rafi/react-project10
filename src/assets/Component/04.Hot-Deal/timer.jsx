    ...
    return (
      <>
        {!isExpired && targetDate && targetTime ? (
          <div className="bread__counter d-flex justify-content-between text-center">
            {Object.entries(remaining).map((el, i) => (
              <div key={i} className="entry">
                <div key={el[1]} className="">
                  <span className="bread__counter-value">{el[1]}</span>
                </div>
                <div className="bread__counter-time">{el[0].toUpperCase()}</div>
              </div>
            ))}
          </div>
        ) : (
